...
var DefaultConsolidateOutputsParams = ConsolidateOutputsParams{
	FeeRate:        uint64(utils.DefaultFeeRateBtcPerKvb),
	MaxInputsCount: 20,
	OutputsCount:   2,
}
...
func (c *client) FindUsedInputs(tx *wire.MsgTx) (map[types.OutPoint]btcjson.ListUnspentResult, error) {
	unspent, err := c.ListUnspent()
	if err != nil {
		return nil, errors.Wrap(err, "failed to get available UTXOs")
	}

	usedInputs := make(map[types.OutPoint]btcjson.ListUnspentResult, len(tx.TxIn))
	for _, inp := range tx.TxIn {
		...
		for _, u := range unspent {
			...
			outPoint := types.OutPoint{TxID: u.TxID, Index: u.Vout}
...
func (c *client) MockTransaction(tx *wire.MsgTx, inputs map[types.OutPoint]btcjson.ListUnspentResult) (*wire.MsgTx, error) {
	mockedTx := tx.Copy()
	...
	for i, inp := range mockedTx.TxIn {
		...
		sig, err := c.helper.MockSignatureScript(scriptDecoded, tx, i, utils.ToUnits(unspent.Amount))
		if err != nil {
...
func (c *client) ConsolidateOutputs(to string, opts ...ConsolidateOutputsOptions) (*wire.MsgTx, [][]byte, error) {
	...
	usedInputs := make(map[types.OutPoint]btcjson.ListUnspentResult, limit)
	totalAmount := int64(0)
	for i := 0; i < limit; i++ {
		...
		usedInputs[types.OutPoint{TxID: unspent[i].TxID, Index: unspent[i].Vout}] = unspent[i]
		totalAmount += utils.ToUnits(unspent[i].Amount)
	}
	...
	for i := range tx.TxIn {
		utxo := usedInputs[types.OutPoint{
		...
		sigHash, err := c.helper.CalculateSignatureHash(scriptDecoded, tx, i, utils.ToUnits(utxo.Amount))
		if err != nil {
